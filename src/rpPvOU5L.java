 ...
 */
//[Mã câu hỏi (qCode): xySvcC2e].  Một chương trình (tạm gọi là RMI Server) cung cấp giao diện cho phép triệu gọi từ xa để quản lý thông tin đơn hàng trong hệ thống thương mại điện tử. Chương trình sẽ ngẫu nhiên tạo ra đối tượng Order với các giá trị ban đầu và cung cấp cho RMI client như sau:
//    Giao diện từ xa:
 ...
 * @author hoang
 */
public class rpPvOU5L {
    public static void main(String[] args) throws Exception{
        String studentCode = "B21DCCN662", qCode = "rpPvOU5L";
        Registry rg = LocateRegistry.getRegistry("203.162.10.109");
        ObjectService sv = (ObjectService) rg.lookup("RMIObjectService");
        Order order = (Order) sv.requestObject(studentCode, qCode);
        System.out.println(order);
        //xử lý phần đầu
        String dau = order.getShippingType().substring(0,2).toUpperCase();
        System.out.println(dau);
        //xử lý phần giữa
        String codeCus = order.getCustomerCode();
        int n = codeCus.length();
        String giua = codeCus.substring(n-3, n);
        System.out.println(giua);
        //xử lý phần cuối : lấy day + month
        String date = order.getOrderDate();
        String month = date.substring(5,7);
        String day = date.substring(8, date.length());
        String cuoi = day +  month;
        System.out.println(cuoi);
        // ghép lại
        String kq = dau+ giua+cuoi;
        System.out.println(kq);
        //set lại 
        // lưu ý bao nhiêu lần truyền nhầm biến kq thành biến khác rồi 
        order.setOrderCode(kq);
        // gửi lên sv
        System.out.println(order);
        sv.submitObject(studentCode, qCode, order);
    }
}
